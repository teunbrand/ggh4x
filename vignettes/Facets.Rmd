...
p + facet_wrap2(vars(year), strip = strip_vanilla(clip = "on")) +
  ggtitle('clip = "on"')
...
p + facet_grid2(long + short ~ ., switch = "y",
...
In contrast, when `bleed = TRUE`, lower layer strips are allowed to 'bleed' into oneanother even though the higher level strips are different. In the example below, you can see that the "f" strips of the 2nd and 3rd panel have merged, even though they have different labels ("4" and "5") in higher layer strips.
